...
def cleanDurationColumn(df):
    ...
    if sum(physicalActivityRows) > 0:
        df["physicalActivityInfo"] = np.nan
        df.loc[physicalActivityRows, "physicalActivityInfo"] = df.loc[physicalActivityRows, 'duration']
...
def createBolusSchedules(df):
    if "bolus" in list(df):
        bolus_schedule_locations = df["bolus"].str.contains("{") == True
        df.loc[bolus_schedule_locations, "bolusSchedules"] = df.loc[bolus_schedule_locations, "bolus"]
        ...
        if scheduleName in list(df):
            ...
            for scheduleNameRow in scheduleNameRows:
                ...
                for scheduleNameKey in scheduleNameKeys:
                    ...
                    hashedScheduleName = \
                    hashlib.sha256((str(scheduleNameKey) + str(args.salt) + str(userID)).
...
def flattenInpenData(df):
    ...
    if len(inpen_idx) > 0:
        ...
        keep_columns = ['Duration Of Insulin Action In Seconds',
                        'Insulin Concentration',
                        'Insulin Name',
                        'HKWasUserEntered']
        
        dict_df = dict_df[keep_columns].copy()
        dict_df.columns = ['durationOfInsulinActionInSeconds',
...
cgm_df, numberOfInvalidCgmValues = removeInvalidCgmValues(cgm_df)
cgm_df, nDuplicatesRemovedDeviceTime = removeCgmDuplicates(cgm_df, "deviceTime")
cgm_df, nDuplicatesRemovedUtcTime = removeCgmDuplicates(cgm_df, "time")
...
cgm_df, nDuplicatesRemovedRoundedTime = removeDuplicates(cgm_df, "roundedTime")
