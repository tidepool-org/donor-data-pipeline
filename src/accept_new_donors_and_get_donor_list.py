    ...
    if(api_response.ok):
        ...
        for i in range(0, len(usersData)):
            ...
            if(api_response2.ok):
                ...
                nAccepted = 0
                print("Error with accepting invites " +
                    str(api_response2.status_code)
                )
...
def accept_and_get_list():
    ...
    # additional accounts to ignore (QA testing)
    accounts_to_ignore = [
    ...
    ignore_id_list = pd.read_csv('ignore_id_list.csv', low_memory=False)
    ...
    for donor_group in donor_groups:
        ...
        print("DONOR GROUP ~~~~~~ : " + str(donor_group))
