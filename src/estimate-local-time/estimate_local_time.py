...
def reorderColumns(cDF):
    column_ordering = set(["pump.upload.imputed.timezoneOffset",
    ...
    dataframe_columns = set(cDF)
    missing_columns_from_ordering = column_ordering - dataframe_columns
    remaining_column_ordering = list(column_ordering - missing_columns_from_ordering)
    cDF = cDF[remaining_column_ordering]
...
def addHealthKitTimezones(df):

    HK_tz_df = df[df.payload.astype(str).str.contains('HKTimeZone')]

    if len(HK_tz_df) > 0:
        eval_df = HK_tz_df.payload.astype(str).apply(ast.literal_eval)
        dict_df = pd.DataFrame.from_records(eval_df.values, index=eval_df.index)
        df.loc[HK_tz_df.index, 'timezone'] = dict_df['HKTimeZone']

    return df


def addHealthKitTimezoneOffsets(df):

    HK_tz_df = df[df.payload.astype(str).str.contains('HKTimeZone')]

    if len(HK_tz_df) > 0:
        df['timezoneOffset'] = \
            HK_tz_df.apply(lambda x: getTimezoneOffset(
                                pd.to_datetime(x.utcTime),
                                x.timezone),
                           axis=1)
...
def estimate_local_time(data):

    if os.path.isfile("wikipedia-timezone-aliases-2018-04-28.csv"):
        timezoneAliases = \
            pd.read_csv("wikipedia-timezone-aliases-2018-04-28.csv",
                        low_memory=False)
    else:
    ...
    # This converts HH:MM:SS-#### timestamps into utc time from their offset
    ...
    # add healthkit timezones to timezone field
    data = addHealthKitTimezones(data)

    # convert deprecated timezones to their aliases
    data = convertDeprecatedTimezoneToAlias(data, timezoneAliases)

    # add healthkit timezone offsets to timezoneOffset field
    data = addHealthKitTimezoneOffsets(data)
    ...
    # Convert the estimated local time to YYYY-mm-ddTHH:MM:SS format
    #data['est.localTime'] = \
    #    data['est.localTime'].dt.strftime('%Y-%m-%dT%H:%M:%S')
...
if __name__ == "__main__":
    ...
    inputFilePathAndName = "example-csv.csv"
    data, fileName = checkInputFile(inputFilePathAndName)
